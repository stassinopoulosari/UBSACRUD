...
export const startConfigurationView = (
  ...
  renderConfiguration = () => {
    ...
    if (configuration.lastPushed === undefined) configuration.lastPushed = 0;
    if (configuration.lastUpdated === undefined) configuration.lastUpdated = 0;
      ...
      Object.values(configuration.schoolYears).forEach((schoolYear) => {
        const bounds = schoolYear.bounds,
          ...
          dateComponentSchedulePairs = everyDateComponent.map(
            (dateComponent) => [
              dateComponent,
              getScheduleForDateComponent(configuration, dateComponent)[0] ??
                "",
            ],
          );
        // Remove past school years
        if (compareDateStrings(bounds.endDate, dateToString(new Date())) < 0)
          return;
        dateComponentSchedulePairs.forEach((pair) => {
          ...
          if (calendar[year][month] === undefined) calendar[year][month] = {};
          calendar[year][month][date] = schedule;
        });
      });
      for (const yearKey in calendar) {
        for (const monthKey in calendar[yearKey]) {
          const components = [parseInt(yearKey), parseInt(monthKey), 1];
    ...
    if (configuration.schoolYears === undefined) configuration.schoolYears = {};
  ...
  renderBounds = (bounds, isNewSchoolYear) => {
    if (isNewSchoolYear) {
      ui.style(ui.update($page.bounds.createSchoolYear, { disabled: true }), {
        display: "block",
      });
    } else {
    ...
    $page.bounds.startDate.onkeyup = $page.bounds.endDate.onkeyup = () => {
      if (
        ...
        if (isNewSchoolYear) {
          ui.update($page.bounds.createSchoolYear, { disabled: false });
        } else {
            ...
            Object.keys(schoolYears).every(
              (schoolYearKey) =>
                ...
                !stringHasOverlap(
                  ...
                  [$page.bounds.startDate.value, $page.bounds.endDate.value],
                ),
            )
          ) {
      ...
      if (isNewSchoolYear) {
        if (
          ...
          if (
            ...
            notifyChange();
            navigateCalendar(
              $page.bounds.startDate.value.split("-").slice(0, 2).join("-"),
              configuration,
            );
            renderConfiguration();
            $page.bounds.error.innerHTML = "&nbsp;";
    ...
    ui.children(
      ...
      DAYS_OF_WEEK.map((day) => {
        ...
        $saveButton.onclick = () => {
          ...
          configuration.schoolYears[currentSchoolYear].defaultWeek[day] =
            $scheduleSelect.value === "" ? null : $scheduleSelect.value;
          notifyChange();
  ...
  renderBreak = (breakKey, breakObject) => {
    ...
    const $startDate = ui.update(ui.make("input"), {
      ...
      $scheduleSelect = ui.update(renderScheduleSelect(ui.make("select")), {
      ...
      if (
        ...
        if (
          stringOnInterval(
            $startDate.value,
            [
              configuration.schoolYears[currentSchoolYear].bounds.startDate,
              configuration.schoolYears[currentSchoolYear].bounds.endDate,
            ],
            true,
          ) &&
          stringOnInterval(
          ...
          breakObject = configuration.schoolYears[currentSchoolYear].breaks[
            breakKey
          ] = {
          ...
          $page.breaks.error.innerHTML = "break is out of school year bounds";
        }
      } else {
    ...
    $deleteButton.onclick = () => {
      delete configuration.schoolYears[currentSchoolYear].breaks[breakKey];
      notifyChange();
      reloadCalendar(configuration);
      $parent.remove();
    };
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            const key = generateConfigurationKey(currentSchoolYear, "breaks");
            configuration.schoolYears[currentSchoolYear].breaks[key] = {
              startDate: $page.breaks.add.startDate.value,
              endDate: $page.breaks.add.endDate.value,
              defaultSchedule: $page.specialSchedules.add.schedule.value,
            };
            notifyChange();
            reloadCalendar(configuration);
    ...
    $saveButton.onclick = () => {
      if ($dateInput.value !== "" && $scheduleSelect.value !== "") {
        if (
          ...
          $page.specialSchedules.error.innerHTML = "out of school year bounds";
        }
      } else {
        $page.specialSchedules.error.innerHTML = "special schedule is invalid";
      }
    };
    $deleteButton.onclick = () => {
      delete configuration.schoolYears[currentSchoolYear].specialSchedules[
        specialScheduleKey
      ];
      notifyChange();
      reloadCalendar(configuration);
    ...
    $page.specialSchedules.add.submit.onclick = () => {
      if (
        ...
        if (
          ...
          configuration.schoolYears[currentSchoolYear].specialSchedules[key] = {
          ...
          ui.children($page.specialSchedules.list, [
            renderSpecialSchedule(
              key,
              configuration.schoolYears[currentSchoolYear].specialSchedules[
                key
              ],
            ),
          ]);
        ...
        $page.specialSchedules.add.error.innerHTML = "input is incomplete";
      }
    };
