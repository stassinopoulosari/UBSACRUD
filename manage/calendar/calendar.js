...
let currentMonth = dateToDateString(currentDate)
	.split("-")
	.slice(0, 2)
	.join("-");
...
export const renderCalendarView = (configuration, schedules) => {
	...
	createCalendar = (iterator) => {
		...
		for (let weekIndex = 0; weekIndex < NUM_WEEKS; weekIndex++) {
			...
			for (let dayIndex = 0; dayIndex < DAYS_PER_WEEK; dayIndex++) {
				const $dayElement = ui.classes(ui.make("div"), [
					"calendar-day",
				]);
	...
	renderCalendar = (month, configuration, scheduleTitles) => {
		...
		const firstDay = `${month}-01`,
			firstDayComponents = dateStringToDateComponents(firstDay),
		...
		const nextMonthDateString = dateToDateString(nextMonthFirstDate)
				.split("-")
				.slice(0, 2)
				.join("-"),
			prevMonthDateString = dateToDateString(prevMonthLastDate)
				.split("-")
				.slice(0, 2)
				.join("-");
		...
		createCalendar(($dayElement, dayIndex) => {
			...
			const dayOfMonth = runningDate.getDate(),
				dateString = dateToDateString(runningDate),
				scheduleForDay = getScheduleForDateString(
					configuration,
					dateString,
				),
