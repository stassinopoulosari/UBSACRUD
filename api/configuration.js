...
export const getScheduleForDateString = (configuration, dateString) => {
		const matchingSchoolYears = Object.values(
			...
			return dateStringIsOnInterval(dateString, schoolYearInterval, true);
		});
		...
		// First, match breaks
		const breaks =
			schoolYearObject.breaks === undefined
				? {}
				: schoolYearObject.breaks;
		for (const breakObject of Object.values(breaks)) {
			...
			if (dateStringIsOnInterval(dateString, breakInterval, true))
				return [
					breakSchedule,
					{
						matched: "break",
		...
		for (const specialScheduleObject of Object.values(specialSchedules)) {
			if (
				compareDateStrings(specialScheduleObject.date, dateString) === 0
			)
				return [
		...
		const defaultWeekObject = schoolYearObject.defaultWeek ?? {},
			dayOfWeek = DAYS_OF_WEEK[dateStringToDate(dateString).getDay()];
		return [
