      ...
      getSchedules(instanceRef)
        .then((schedules) => {
          ...
          Object.values(schedules).forEach((schedule) => {
            if (symbolExists) return;
            for (const periodKey in schedule) {
              if (periodKey === "name" || periodKey === "hidden") continue;
              if (typeof schedule[periodKey].name !== "string") continue;
              if (schedule[periodKey].name.includes(`$(${symbolKey})`)) {
                symbolExists = true;
                break;
              }
            }
    ...
    new Promise((resolve, reject) =>
      getSchedule(instanceRef, scheduleKey).then((remoteSchedule) => {
        const promiseChain = periods
          ...
          .map((period) => () => addPeriod(instanceRef, scheduleKey, period))
          .concat([() => resolve()]);
